...
const fs       = require             ('fs'                    )            ;
const stream   = fs.createWriteStream("data_output_adams.txt" )            ;
const readline = require             ('readline'              )            ;
const numeric  = require             ('numeric'               )            ;
const os       = require             ("os"                    )            ;
const opn      = require             ('opn'                   )            ;
const stream_graph   = fs.createWriteStream("../graph_adams.txt" )            ;
...
function adams(interval, step, st_point_y, st_point_z) {
  let a = interval[0];
  let b = interval[1];
  let n = (b - a) / step + 1;
  ...
  for (let i = 4; i < n; i++){
    result[1].push( result[1][i - 1] + (step / 24) 
      * (55 * f(result[0][i - 1], result[1][i - 1], result[2][i - 1]) 
      - 59 * f(result[0][i - 1], result[1][i - 1], result[2][i - 1])
      + 37 * f(result[0][i - 1], result[1][i - 1], result[2][i - 1]) 
      - 9 * f(result[0][i - 1], result[1][i - 1], result[2][i - 1])) );
   
      result[2].push( result[2][i - 1] + (step / 24) 
      * (55 * g(result[0][i - 1], result[1][i - 1], result[2][i - 1]) 
      - 59 * g(result[0][i - 1], result[1][i - 1], result[2][i - 1])
      + 37 * g(result[0][i - 1], result[1][i - 1], result[2][i - 1]) 
      - 9 * g(result[0][i - 1], result[1][i - 1], result[2][i - 1])) );   
  }
...
function runge_romberg(out, out_less_step, p ) {
  let n = out_less_step.length;
  let result = [];

  for (var i = 0; i < n; i++) {
      result.push( (out[i] - out_less_step[i]) / ( math.pow(2,p) - 1) );
  }
...
function main(start, end, step, start_point_y, start_point_z) {
  ...
  const output = adams([start, end], step, start_point_y, start_point_z);
  const output_less = adams([start, end], step * 2, start_point_y, start_point_z);
  ...
  return [output[0], output[1], ex, output_less[1]];
}
...
const output = main(start, end, step, start_point_y, start_point_z );
const runge_error = runge_romberg(output[1], output[3], 4)
console.log(`runge error = ${runge_error}`);
